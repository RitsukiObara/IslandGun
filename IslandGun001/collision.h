...
namespace collision
{
	...
	// ヤシの実との当たり判定
	void PalmFruitHit(CPlayer* pPlayer, const D3DXVECTOR3& size);			// ヤシの木との当たり判定
	...
	// 風攻撃との当たり判定
	bool WindShotHitToPlayer(CPlayer* pPlayer, const float fRadius, const float fHeight);		// 風攻撃とプレイヤーの当たり判定
