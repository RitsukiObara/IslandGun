...
// 静的メンバ変数宣言
//--------------------------------------------
CRockManager* CRockManager::m_pManager = nullptr;			// プレイヤーの変数
...
// 登録処理
//============================
void CRockManager::Regist(CRock* pThis)
{
	if (m_pTop == nullptr)
	{ // オブジェクトが NULL の場合
	...
	{ // 上記以外
		...
		// ローカルポインタを宣言
		CRock* pObject = m_pTop;			// 先頭のオブジェクト
...
{
	// ローカル変数宣言
	CRock* pObj = nullptr;		// 現在のオブジェクトのポインタ
...
// オブジェクトの取得処理
//===========================================
CRock* CRockManager::GetTop(void)
{
	// オブジェクトの情報を渡す
	return m_pTop;
}
...
// リスト構造の引き抜き処理
//===========================================
void CRockManager::Pull(CRock* pThis)
{
	// ローカル変数宣言
	CRock* pObj = nullptr;		// 現在のオブジェクトのポインタ
...
//生成処理
//============================
CRockManager* CRockManager::Create(void)
{
	if (m_pManager == nullptr)
	{ // オブジェクトが NULL の場合

		// オブジェクトを生成
		m_pManager = new CRockManager;
	}
	else
	{ // オブジェクトが NULL じゃない場合
...
// 取得処理
//============================
CRockManager* CRockManager::Get(void)
{
	// マネージャーのポインタを返す
	return m_pManager;
}